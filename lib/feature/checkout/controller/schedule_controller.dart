...
import '../../../utils/time_picker_widget/time_picker_widget.dart';

class ScheduleController extends GetxController {
  ///Selected date of day
  DateTime _selectedDate =
      DateTime.now().add(Duration(hours: AppConstants.SCHEDULE_TIME));
  ...
  Future<void> selectTimeOfDay() async {
    ...
    //     context: Get.context!,
    //     initialTime: TimeOfDay(hour: DateTime.now().hour + AppConstants.SCHEDULE_TIME, minute: 0));
    ...
    final TimeOfDay? pickedTime = await showCustomTimePicker(
        ...
        initialTime: TimeOfDay(hour: DateTime.now().hour, minute: xvalue),
    ...
    if (pickedTime != null) {
      _selectedDate = DateTime(_selectedDate.year, _selectedDate.month,
          _selectedDate.day, pickedTime.hour);
      update();
  ...
  bool checkScheduleTime() {
    return _selectedDate.difference(DateTime.now()) >
        Duration(hours: AppConstants.SCHEDULE_TIME, minutes: -15);
  }
