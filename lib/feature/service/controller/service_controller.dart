...
class ServiceController extends GetxController implements GetxService {
  ...
  Future<void> getCampaignBasedServiceList(String campaignID, bool isWithPagination) async {
    printLog("inside_campaign_based_service !");
    if(!isWithPagination){
    ...
    if (response.statusCode == 200) {
      ...
      _isLoading = false;
      print("_campaignBasedServiceList${_campaignBasedServiceList!.length}");
  ...
  int setExistInCart(Service service, {bool notify = true}) {
    ...
    // _cartIndex = Get.find<CartController>().isExistInCart(service.id!, variationType, false);
    if(_cartIndex != -1) {
