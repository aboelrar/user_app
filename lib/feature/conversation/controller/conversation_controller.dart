...
class ConversationController extends GetxController implements GetxService{
  ...
  void pickMultipleImage(bool isRemove,{int? index}) async {
    print("inside_pick_multiple_image");
    if(isRemove) {
      if(index != null){
  ...
  Future<void> getChannelListBasedOnReferenceId(int offset,String referenceId) async{
    print("inside_channel_list_based_reference_id");
    _channelList = [];
  ...
  Future<void> sendMessage(String channelID) async{
    ...
    _isLoading = true;
    update();
    Response response = await conversationRepo.sendMessage(conversationController.value.text,channelID ,_selectedImageList, _file);
    if(response.statusCode == 200){
      _isLoading = false;
