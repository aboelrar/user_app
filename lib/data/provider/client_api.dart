...
class ApiClient extends GetxService {
  ...
  Future<Response> postMultipartDataConversation(
    ...
    if(otherFile != null) {
      print("inside_other_file");
      Uint8List _list = await otherFile.readAsBytes();
      var part = Http.MultipartFile('files[${multipartBody!.length}]', otherFile.readAsBytes().asStream(), _list.length, filename: basename(otherFile.path));
      _request.files.add(part);
    }
    if(multipartBody!=null){
      print("inside_multipart_body");
    ...
    _request.files.forEach((element) {
      print("fileName__1:${element.filename}");
    });
